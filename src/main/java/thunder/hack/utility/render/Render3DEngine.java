...
public class Render3DEngine {
    ...
    // Starter ESP анимация
    private static float starterEspAnimation = 0f;
    private static float starterEspTargetAnim = 0f;
    private static Entity lastStarterTarget = null;
    ...
    public static void drawTargetEsp(MatrixStack stack, @NotNull Entity target) {
        ...
        for (int j = 0; j < vecs.size() - 1; ++j) {
            float alpha = 1f - (((float) j + ((System.currentTimeMillis() - HolyFacker.initTime) / 5f)) % 360) / 60f;
    ...
    public static void updateStarterEspAnimation(Entity target) {
        if (target != null) {
            starterEspTargetAnim = 1f;
            lastStarterTarget = target;
        } else {
            starterEspTargetAnim = 0f;
        }
        // Плавная интерполяция
        starterEspAnimation += (starterEspTargetAnim - starterEspAnimation) * 0.15f;
        if (starterEspAnimation < 0.01f) starterEspAnimation = 0f;
        if (starterEspAnimation > 0.99f) starterEspAnimation = 1f;
    }

    public static float getStarterEspAnimation() {
        return starterEspAnimation;
    }

    public static Entity getLastStarterTarget() {
        return lastStarterTarget;
    }
    ...
    // Starter TargetESP - яркий круг под ногами с анимацией
    public static void drawStarterTargetEsp(MatrixStack stack, @NotNull Entity target) {
        float anim = starterEspAnimation;
        ...
        float baseRadius = target.getWidth() * 0.9f;
        
        // Анимация появления - радиус растет от 0
        float radius = baseRadius * anim;
        // Анимация прозрачности
        float alphaMultiplier = anim;

        stack.push();
        stack.translate(x, y + 0.02, z);
        ...
        // Яркий заполненный круг
        BufferBuilder buffer = Tessellator.getInstance().begin(VertexFormat.DrawMode.TRIANGLE_FAN, VertexFormats.POSITION_COLOR);
        Color centerColor = HudEditor.getColor(0);
        buffer.vertex(matrix, 0, 0, 0).color(Render2DEngine.applyOpacity(centerColor.brighter(), alphaMultiplier).getRGB());
        
        for (int i = 0; i <= segments; i++) {
            float angle = (float) (i * Math.PI * 2 / segments);
            float cos = (float) Math.cos(angle);
            float sin = (float) Math.sin(angle);
            Color edgeColor = HudEditor.getColor(i * 6);
            buffer.vertex(matrix, cos * radius, 0, sin * radius).color(Render2DEngine.applyOpacity(edgeColor, 0.5f * alphaMultiplier).getRGB());
        }
        BufferRenderer.drawWithGlobalProgram(buffer.end());

        // Яркая внешняя обводка
        BufferBuilder outline = Tessellator.getInstance().begin(VertexFormat.DrawMode.TRIANGLE_STRIP, VertexFormats.POSITION_COLOR);
        for (int i = 0; i <= segments; i++) {
            float angle = (float) (i * Math.PI * 2 / segments);
            float cos = (float) Math.cos(angle);
            float sin = (float) Math.sin(angle);
            Color color = HudEditor.getColor(i * 6).brighter();
            outline.vertex(matrix, cos * radius, 0, sin * radius).color(Render2DEngine.applyOpacity(color, alphaMultiplier).getRGB());
            outline.vertex(matrix, cos * (radius + 0.15f * anim), 0, sin * (radius + 0.15f * anim)).color(Render2DEngine.applyOpacity(color, 0.3f * alphaMultiplier).getRGB());
        }
        BufferRenderer.drawWithGlobalProgram(outline.end());

        // Пульсирующее расширяющееся кольцо (только если анимация почти завершена)
        if (anim > 0.8f) {
            float ringAnim = (anim - 0.8f) / 0.2f;
            float expandRadius = radius + 0.3f + time * 0.7f;
            float expandAlpha = (1f - time) * ringAnim;
            
            BufferBuilder ring = Tessellator.getInstance().begin(VertexFormat.DrawMode.TRIANGLE_STRIP, VertexFormats.POSITION_COLOR);
            for (int i = 0; i <= segments; i++) {
                float angle = (float) (i * Math.PI * 2 / segments);
                float cos = (float) Math.cos(angle);
                float sin = (float) Math.sin(angle);
                Color color = HudEditor.getColor(i * 6);
                ring.vertex(matrix, cos * expandRadius, 0, sin * expandRadius).color(Render2DEngine.applyOpacity(color, expandAlpha * 0.8f).getRGB());
                ring.vertex(matrix, cos * (expandRadius + 0.1f), 0, sin * (expandRadius + 0.1f)).color(Render2DEngine.applyOpacity(color, 0).getRGB());
            }
            BufferRenderer.drawWithGlobalProgram(ring.end());

            // Второе пульсирующее кольцо (со сдвигом)
            float time2 = ((System.currentTimeMillis() + 750) % 1500) / 1500f;
            float expandRadius2 = radius + 0.3f + time2 * 0.7f;
            float expandAlpha2 = (1f - time2) * ringAnim;
            
            BufferBuilder ring2 = Tessellator.getInstance().begin(VertexFormat.DrawMode.TRIANGLE_STRIP, VertexFormats.POSITION_COLOR);
            for (int i = 0; i <= segments; i++) {
                float angle = (float) (i * Math.PI * 2 / segments);
                float cos = (float) Math.cos(angle);
                float sin = (float) Math.sin(angle);
                Color color = HudEditor.getColor(i * 6);
                ring2.vertex(matrix, cos * expandRadius2, 0, sin * expandRadius2).color(Render2DEngine.applyOpacity(color, expandAlpha2 * 0.8f).getRGB());
                ring2.vertex(matrix, cos * (expandRadius2 + 0.1f), 0, sin * (expandRadius2 + 0.1f)).color(Render2DEngine.applyOpacity(color, 0).getRGB());
            }
            BufferRenderer.drawWithGlobalProgram(ring2.end());
        }
    ...
    // Рендер с анимацией исчезновения (для последней цели)
    public static void drawStarterTargetEspFadeOut(MatrixStack stack) {
        if (starterEspAnimation > 0.01f && starterEspAnimation < 0.99f && lastStarterTarget != null && starterEspTargetAnim == 0f) {
            drawStarterTargetEsp(stack, lastStarterTarget);
        }
    }
