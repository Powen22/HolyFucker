...
public class TargetHud extends HudElement {
    ...
    private void renderMiniNurik(DrawContext context, float health, float animationFactor) {
        ...
        String name = ModuleManager.media.isEnabled() ? "Protected " : ModuleManager.nameProtect.isEnabled() && target == mc.player ? NameProtect.getCustomName() : target.getName().getString();
        float maxNameWidth = 52; // Максимальная ширина для ника
        if (FontRenderers.sf_bold_mini.getStringWidth(name) > maxNameWidth) {
            while (FontRenderers.sf_bold_mini.getStringWidth(name + "..") > maxNameWidth && name.length() > 1) {
                name = name.substring(0, name.length() - 1);
            }
            name = name + "..";
        }
        FontRenderers.sf_bold_mini.drawString(context.getMatrices(), name, getPosX() + 38, getPosY() + 5, Render2DEngine.applyOpacity(Colors.WHITE, animationFactor));
    ...
    private void renderStarter(DrawContext context, float health, float animationFactor) {
        float hurtPercent = (Render2DEngine.interpolateFloat(MathUtility.clamp(target.hurtTime == 0 ? 0 : target.hurtTime + 1, 0, 10), target.hurtTime, Render3DEngine.getTickDelta())) / 8f;
        healthAnimation.setValue(health);
        health = (float) healthAnimation.getAnimationD();
        ...
        // Основной фон
        Render2DEngine.drawRound(context.getMatrices(), getPosX(), getPosY(), width, height, 7, new Color(0, 0, 0, 140));
        
        float healthPercent = MathUtility.clamp(health / target.getMaxHealth(), 0, 1);
        Color accentColor = Render2DEngine.interpolateColorC(new Color(255, 60, 60), new Color(80, 255, 120), healthPercent);
        ...
        boolean winning = myHealth > targetHealth;
        String statusText = winning ? "W" : "L";
        Color statusColor = winning ? new Color(80, 255, 120) : new Color(255, 80, 80);
        
        // Статус справа вверху
        FontRenderers.sf_bold_mini.drawString(context.getMatrices(), statusText, getPosX() + width - 10, getPosY() + 3, Render2DEngine.applyOpacity(statusColor.getRGB(), animationFactor));
        ...
        float headSize = 27;
        float headX = getPosX() + 4;
        float headY = getPosY() + 4;

        context.getMatrices().push();
        context.getMatrices().translate(headX + headSize / 2, headY + headSize / 2, 0);
        context.getMatrices().scale(1 - hurtPercent / 12f, 1 - hurtPercent / 12f, 1f);
        context.getMatrices().translate(-(headX + headSize / 2), -(headY + headSize / 2), 0);
        ...
        RenderSystem.setShader(GameRenderer::getPositionColorProgram);
        Render2DEngine.renderRoundedQuadInternal(context.getMatrices().peek().getPositionMatrix(), animationFactor, animationFactor, animationFactor, animationFactor,
                headX, headY, headX + headSize, headY + headSize, 5, 10);
        RenderSystem.blendFunc(GL40C.GL_DST_ALPHA, GL40C.GL_ONE_MINUS_DST_ALPHA);
        RenderSystem.setShaderColor(animationFactor, animationFactor - hurtPercent / 2, animationFactor - hurtPercent / 2, animationFactor);
        Render2DEngine.renderTexture(context.getMatrices(), headX, headY, headSize, headSize, 8, 8, 8, 8, 64, 64);
        Render2DEngine.renderTexture(context.getMatrices(), headX, headY, headSize, headSize, 40, 8, 8, 8, 64, 64);
        RenderSystem.setShaderColor(1f, 1f, 1f, 1f);
        RenderSystem.defaultBlendFunc();
        context.getMatrices().pop();
        ...
        // Полоска HP
        float barX = getPosX() + 35;
        float barY = getPosY() + 24;
        float barWidth = 55;
        float barHeight = 6;

        // Фон полоски
        Render2DEngine.drawRound(context.getMatrices(), barX, barY, barWidth, barHeight, 3, new Color(255, 255, 255, 25));

        // Заполненная часть
        float filledWidth = MathUtility.clamp(barWidth * healthPercent, 4, barWidth);
        Color startColor = new Color(255, 100, 100);
        Color endColor = new Color(100, 255, 150);
        Color currentColor = Render2DEngine.interpolateColorC(startColor, endColor, healthPercent);
        Color darkerColor = currentColor.darker();
        
        Render2DEngine.drawGradientRound(context.getMatrices(), barX, barY, filledWidth, barHeight, 3, currentColor, darkerColor, darkerColor, currentColor);
    }
