...
public class Velocity extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        ...
        if (e.getPacket() instanceof EntityVelocityUpdateS2CPacket pac) {
            if (pac.getId() == mc.player.getId() && (!onlyAura.getValue() || ModuleManager.aura.isEnabled())) {
                switch (mode.getValue()) {
                    ...
                    case GrimAdvanced -> {
                        // Advanced Grim bypass - reduces velocity very subtly to stay within offset tolerance
                        // Grim checks offset (difference between predicted and actual movement)
                        // Threshold is 0.001, so we need to reduce velocity very carefully
                        
                        // Calculate original velocity
                        double velX = pac.getVelocityX() / 8000.0;
                        double velY = pac.getVelocityY() / 8000.0;
                        double velZ = pac.getVelocityZ() / 8000.0;
                        
                        // Calculate velocity magnitude
                        double magnitude = Math.sqrt(velX * velX + velY * velY + velZ * velZ);
                        
                        // Only reduce if velocity is significant
                        if (magnitude > 0.001) {
                            // Reduce velocity by very small amount (5-10% with randomization)
                            // This keeps offset within Grim's tolerance (0.001)
                            double reduction = 0.92 + (Math.random() * 0.03); // 92-95% reduction
                            
                            // Apply reduction
                            double newX = velX * reduction;
                            double newY = velY * reduction;
                            double newZ = velZ * reduction;
                            
                            // Convert back to packet format (multiply by 8000)
                            ((ISPacketEntityVelocity) pac).setMotionX((int) (newX * 8000.0));
                            ((ISPacketEntityVelocity) pac).setMotionY((int) (newY * 8000.0));
                            ((ISPacketEntityVelocity) pac).setMotionZ((int) (newZ * 8000.0));
                        } else {
                            // If velocity is too small, cancel it completely
                            e.cancel();
                        }
                    }
                }
        ...
        if (e.getPacket() instanceof ExplosionS2CPacket explosion && explosions.getValue()) {
            switch (mode.getValue()) {
                ...
                case GrimAdvanced -> {
                    // Reduce explosion velocity subtly
                    float reduction = (float)(0.90 + (Math.random() * 0.05)); // 90-95% reduction
                    ((IExplosionS2CPacket) explosion).setMotionX(((IExplosionS2CPacket) explosion).getMotionX() * reduction);
                    ((IExplosionS2CPacket) explosion).setMotionY(((IExplosionS2CPacket) explosion).getMotionY() * reduction);
                    ((IExplosionS2CPacket) explosion).setMotionZ(((IExplosionS2CPacket) explosion).getMotionZ() * reduction);
                }
            }
        }
        ...
        if (e.getPacket() instanceof PlayerPositionLookS2CPacket) {
            if (cc.getValue() || mode.getValue() == modeEn.GrimNew || mode.getValue() == modeEn.GrimAdvanced)
                ccCooldown = 5;
        }
    }
    ...
    public void onUpdate() {
        ...
        switch (mode.getValue()) {
            ...
            case GrimAdvanced -> {
                // GrimAdvanced modifies packets directly, no additional handling needed
                // The reduced velocity is applied naturally through the modified packet
            }
        }
        if (grimTicks > 0)
            grimTicks--;
    ...
    public enum modeEn {
        Matrix, Cancel, Sunrise, Custom, Redirect, OldGrim, Jump, GrimNew, GrimAdvanced
    }
