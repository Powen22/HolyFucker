...
public class Aura extends Module {
    ...
    /*   ADVANCED   */
    public final Setting<SettingGroup> advanced = new Setting<>("Advanced", new SettingGroup(true, 0));
    ...
    private void calcRotations(boolean ready) {
        ...
        pitchAcceleration = Managers.PLAYER.checkRtx(rotationYaw, rotationPitch, getRange() + aimRange.getValue(), getRange() + aimRange.getValue(), rayTrace.getValue())
                ? aimedPitchStep : pitchAcceleration < maxPitchStep ? pitchAcceleration * pitchAccelerate : maxPitchStep;
        ...
        float yawStep = rotationMode.getValue() != Mode.Track ? 360f : random(minYawStep, maxYawStep);
    ...
    private float getFOVAngle(@NotNull LivingEntity e) {
        ...
        return Math.abs(yaw - wrapDegrees(mc.player.getYaw()));
    }
