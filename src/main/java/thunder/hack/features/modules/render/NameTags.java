...
public class NameTags extends Module {
    ...
    private final Setting<Boolean> offhandItem = new Setting<>("OffhandItem", true);
    ...
    public void onRender2D(DrawContext context) {
        ...
        for (PlayerEntity ent : mc.world.getPlayers()) {
            ...
            // Проверяем Discord имена
            String playerName = null;
            DiscordNameplates discordModule = ModuleManager.discordNameplates;
            if (discordModule != null && 
                discordModule.isOn() && 
                discordModule.enableDiscordNameplates.getValue()) {
                // Убеждаемся, что сервис инициализирован
                if (discordModule.nameService == null) {
                    discordModule.initializeService();
                }
                if (discordModule.nameService != null) {
                    String discordName = discordModule.getDiscordName(ent);
                    if (discordName != null && !discordName.isEmpty()) {
                        playerName = discordModule.formatDiscordName(discordName);
                    }
                }
            }
            
            // Если Discord имя не найдено, используем обычное имя
            if (playerName == null || playerName.isEmpty()) {
                playerName = ent.getDisplayName().getString();
            }
            
            // Проверяем NameProtect для друзей
            ...
            if (position != null) {
                ...
                float boxX = tagX - 4;
                float boxY = (float) (posY - 15f);
                float boxWidth = textWidth + 8;
                float boxHeight = 14;
                
                // Градиентная подсветка
                if (cl == OutlineColor.Sync || cl == OutlineColor.New) {
                    Render2DEngine.drawGradientBlurredShadow1(context.getMatrices(), boxX + 2, boxY + 2, boxWidth - 4, boxHeight - 4, 6,
                    ...
                    // Темный фон
                    Render2DEngine.drawRound(context.getMatrices(), boxX + 1f, boxY + 1f, boxWidth - 2, boxHeight - 2, 3, new Color(0, 0, 0, 200));
                } else if (cl == OutlineColor.Custom) {
                    Color outColor = Managers.FRIEND.isFriend(ent) ? friendOutlineColor.getValue().getColorObject() : outlineColor.getValue().getColorObject();
                    Render2DEngine.drawBlurredShadow(context.getMatrices(), boxX, boxY, boxWidth, boxHeight, 5, outColor);
                    Render2DEngine.drawRound(context.getMatrices(), boxX, boxY, boxWidth, boxHeight, 4, outColor);
                    Render2DEngine.drawRound(context.getMatrices(), boxX + 1f, boxY + 1f, boxWidth - 2, boxHeight - 2, 3, new Color(0, 0, 0, 200));
                } else {
                    Render2DEngine.drawRound(context.getMatrices(), boxX, boxY, boxWidth, boxHeight, 4, color);
                }

                if (Managers.TELEMETRY.getOnlinePlayers().contains(ent.getGameProfile().getName())) {
                    Render2DEngine.drawRound(context.getMatrices(), boxX - 14, boxY, 12, boxHeight, 4, color.brighter().brighter());
                    ...
                    Render2DEngine.renderTexture(context.getMatrices(), boxX - 13, boxY + 2, 10, 10, 0, 0, 256, 256, 256, 256);
                ...
                // Текст
                if (font.getValue() == Font.Fancy) {
                    FontRenderers.sf_bold.drawString(context.getMatrices(), final_string, tagX, (float) posY - 11, -1);
                } else {
                    context.getMatrices().push();
                    context.getMatrices().translate(tagX, ((float) posY - 12), 0);
                    context.drawText(mc.textRenderer, final_string, 0, 0, -1, false);
                    context.getMatrices().pop();
                }
                
                // Предмет в оффхенде
                if (offhandItem.getValue() && !ent.getOffHandStack().isEmpty()) {
                    net.minecraft.text.Text offhandText = ent.getOffHandStack().getName();
                    int offhandWidth = mc.textRenderer.getWidth(offhandText);
                    float offhandBoxX = (float) (posX + diff - offhandWidth / 2) - 3;
                    float offhandBoxY = boxY + boxHeight + 2;
                    
                    Render2DEngine.drawRound(context.getMatrices(), offhandBoxX, offhandBoxY, offhandWidth + 6, 10, 3, new Color(0, 0, 0, 180));
                    context.drawTextWithShadow(mc.textRenderer, offhandText, (int) (posX + diff - offhandWidth / 2), (int) offhandBoxY + 1, 0xFFFFFF);
                }
